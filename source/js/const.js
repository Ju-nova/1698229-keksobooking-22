const AMOUNT_ADVERT = 10;
const DELAY_TIME = 500;
const DELAY_TIME_CLOSE_MESSAGE = 5000;
const GET_DATA_URL = 'https://22.javascript.pages.academy/keksobooking/data';
const SEND_DATA_URL = 'https://22.javascript.pages.academy/keksobooking';
const STYLE_MAP = 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';
const COPYRIGTH_MAP = '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors';
const ANY = 'any';
const centerCoordinates = {
  lat: 35.68955,
  lng: 139.69222,
}

export {AMOUNT_ADVERT,DELAY_TIME, GET_DATA_URL, SEND_DATA_URL, centerCoordinates, STYLE_MAP, COPYRIGTH_MAP, ANY, DELAY_TIME_CLOSE_MESSAGE}
